...
function ENT:Use(pl)
	if pl == self:GetOwner() and pl:Alive() and self:GetVelocity():Length() <= self.HoverSpeed then
		self:OnPackedUp(pl)
	end
end
...
function ENT:OnPackedUp(pl)
	pl:GiveEmptyWeapon("weapon_zs_manhack")
	pl:GiveAmmo(1, "manhack")
...
function ENT:Think()
	...
	if ent and ent:IsValid() then
		if CurTime() >= (self.NextTouch[ent] or 0) then
			diddamage = true
			...
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:AddVelocity(dir * 66)
			end
		else
			local physattacker = ent:GetPhysicsAttacker()
			if physattacker:IsValid() then
	...
	if not diddamage then
		...
		if data.Speed >= self.MaxSpeed * 0.7 and ent and ent:IsWorld() and CurTime() >= self.PhysDamageImmunity then
			self:TakeDamage(math.Clamp(data.Speed * 0.09, 0, 30))
		end
	end
end
