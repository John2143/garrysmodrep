...
function ENT:IsValidTarget(target)
	...
	return target:IsNPC() or target:IsPlayer() and target ~= owner and target:Alive() and self:GetForward():Dot(self:GetAnglesToTarget(target):Forward()) >= self.MinimumAimDot and TrueVisible(self:ShootPos(), self:GetTargetPos(target), self)
end
...
function ENT:GetManualTrace()
	local owner = self:GetObjectOwner()
	return owner:TraceLine(4096, MASK_SOLID, false)
end
