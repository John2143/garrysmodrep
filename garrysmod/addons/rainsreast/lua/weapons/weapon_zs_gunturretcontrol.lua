...
function SWEP:Reload()
	...
	if CLIENT or !turret then return end
	local i = false
	timer.Create("turrettimer"..math.random(1000),.1,4,function()
		local ent = ents.Create( "rpg_missile" )
		if (  !IsValid( ent ) ) then return end
		local vectorthing = turret:GetPos()
		ent:SetPos( vectorthing + turret:GetUp() * 65 + turret:GetRight()*25*(i and -1 or 1))
		ent:SetAngles( turret:GetGunAngles() )
		ent:SetOwner( self.Owner )
		ent:SetSaveValue( "m_flDamage", 33 ) 
		ent:Spawn()
		ent:SetGravity(.1)
